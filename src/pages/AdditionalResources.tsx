...
const FALLBACK = {
  ...
  introAr:
    'استكشف التقارير التكميلية والمبادئ التوجيهية والمواد المرجعية المتعلقة بمؤشر الشمول المالي للاتحاد وجهود القياس الإقليمية.',
...
export default function AdditionalResources() {
  ...
  const resources = Array.isArray(data.resources) ? (data.resources as ResourceLink[]) : [];

  const introText =
    lang === 'ar'
      ? (((data as typeof FALLBACK).introAr) || t(lang, 'additionalResources.intro', FALLBACK.introAr))
    ...
    <div className="bg-gray-50 min-h-screen pb-20">
      ...
      <div className="bg-gradient-to-br from-[#009900] to-[#00b300] text-white py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <span className="inline-block px-4 py-1.5 bg-amber-400 text-[#009900] text-xs font-bold rounded-full mb-6 tracking-widest uppercase">
            {t(lang, 'additionalResources.badge', 'Financial Inclusion Index')}
          </span>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight">
            {t(lang, 'additionalResources.title', 'Additional Resources')}
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-10 relative z-10">
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sm:p-12 lg:p-16 space-y-8">
          <p
            className="text-gray-600 text-base sm:text-lg leading-relaxed max-w-4xl"
            dir={isRtl ? 'rtl' : 'ltr'}
          >
            {introText}
          </p>
          ...
          {resources.length === 0 ? (
            <p className="text-center text-gray-400 py-12">
              {t(lang, 'additionalResources.empty', 'No additional resources have been published yet.')}
            </p>
          ) : (
            <div className="grid gap-4">
              {resources.map((resource, index) => {
                const title = lang === 'ar' && resource.titleAr ? resource.titleAr : resource.title;
                const description =
                  lang === 'ar' && resource.descriptionAr
                ...
                return (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <h2 className="font-bold text-[#009900] group-hover:text-amber-600 transition-colors">
                        {title}
                      </h2>
                      {description && (
