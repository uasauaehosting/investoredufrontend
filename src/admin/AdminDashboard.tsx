...
type Section =
  | 'overview' | 'slides' | 'news' | 'members' | 'portal' | 'education'
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`
        fixed inset-y-0 start-0 z-30 w-64 bg-[#00285e] text-white flex flex-col
      ...
      `}>
        <div className="px-5 py-5 border-b border-white/10">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-200 text-xs mt-0.5">Content Management</p>
            </div>
        ...
        <div className="px-4 py-4 border-t border-white/10 space-y-2">
          <a href="/" className="flex items-center gap-2 text-blue-200 hover:text-white text-xs transition-colors px-2 py-1.5 rounded-lg hover:bg-white/10">
            <ExternalLink size={13} /> View Public Site
          </a>
          <button onClick={signOut} className="w-full flex items-center gap-2 text-blue-200 hover:text-red-300 text-xs transition-colors px-2 py-1.5 rounded-lg hover:bg-red-500/10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map(({ id, label, icon: Icon, desc }) => (
          <button
            ...
            className="group text-start bg-white rounded-2xl border border-gray-200 p-5 hover:border-blue-300 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5"
          >
            <div className="inline-flex items-center justify-center w-10 h-10 bg-[#00285e] text-white rounded-xl mb-3">
              <Icon size={18} />
            </div>
