...
export function NewsCard({ item }: { item: NewsItem }) {
  ...
  const [imgSrc, setImgSrc] = useState(
    item.image ?? 'https://images.unsplash.com/photo-1504711432869-efd597cdd042?auto=format&fit=crop&q=80&w=800',
  );
  ...
  return (
    <article className="relative group bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100 hover:-translate-y-0.5">
      <Link to={`/news/${item.id}`} className="absolute inset-0 z-0 rounded-xl" aria-label={title} />
      <div className="relative z-[1] pointer-events-none">
        <div className="relative h-44 overflow-hidden bg-gray-100">
          <img
            ...
            onError={() => setImgSrc('https://images.unsplash.com/photo-1504711432869-efd597cdd042?auto=format&fit=crop&q=80&w=800')}
          />
        ...
        <div className="p-4">
          ...
          {excerpt && (
            <p className="text-xs text-gray-500 mt-2 leading-relaxed line-clamp-3">{excerpt}</p>
          )}
          <div className="flex flex-wrap items-center gap-3 mt-3">
            <span className="inline-flex items-center gap-1 text-xs text-[#009900] font-medium group-hover:text-amber-600 transition-colors">
              {t(lang, 'news.readMore', 'Read More')}
...
export default function NewsSection() {
  ...
  useEffect(() => {
    api.get('/home/news')
      ...
      .catch(() => { setError(t(lang, 'news.error', 'Failed to load news.')); setLoading(false); });
  }, [lang]);
    ...
    <section className="py-14 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8">
          <div dir={isRtl ? 'rtl' : 'ltr'}>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-2xl font-bold text-[#009900]">
                {t(lang, 'news.sectionTitle', 'Latest News')}
              </h2>
            </div>
            <p className="text-gray-500 text-sm ms-4 ps-3">
              {t(lang, 'news.sectionSubtitle', 'Stay updated with the latest investor education news from Arab markets')}
            </p>
          </div>
          <Link
            to="/news"
          ...
          >
            {t(lang, 'news.viewAll', 'View All News')}
          </Link>
        </div>
